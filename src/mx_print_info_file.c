...
static void g_or_l(t_files *files, t_args *args) {
    if (args->fl[4] != 1) {
        ...
        mx_print_space_file(files->max_user, files->entry_file->stat->user_name);
        mx_printchar(' ');
        mx_printchar(' ');
    }
    if (args->fl[11] != 1) {
        mx_printstr(files->entry_file->stat->group_name);
        mx_print_space_file(files->max_group, files->entry_file->stat->group_name);
        mx_printchar(' ');
        mx_printchar(' ');
    }
...
void mx_print_info_file(t_args *args, t_files *files) {
    ...
    if (files->entry_file->stat->permiss[0] == 'b' 
        ...
        mx_print_space_file(files->max_size, files->entry_file->stat->file_size);
